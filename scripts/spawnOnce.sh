#! /bin/bash
# NOTE:  The scrpt needs to return -- run long-running process in the background  
PATH=$HOME/.xmonad/scripts:$PATH
xmonadctl -a goto 1
set -x
export PATH=~/bin:/usr/lucho/bin:/usr/local/bin:/bin:$PATH
xrdb -merge ~/.Xresources
setxkbmap -option "compose:lwin"
setxkbmap -option "caps:none"
xsetroot -bg 'gray30' -fg '#c0c0b0'  -mod 16 16

## Background
#fbsetbg -t /usr/share/backgrounds/larry-the-cow/gentoo-larry-bg-1920x1080.png

## Spawn two root terminals
xmonadctl -a goto 1
xmonadctl -a goto left
sudo -u toor env -u XDG_RUNTIME_DIR terminator&
sleep  1
xmonadctl -a goto right
sudo -u toor env -u XDG_RUNTIME_DIR terminator&
sleep 1

# Mount encrypted directories; and launh a root term while we still have su.
if ! mountpoint "${HOME}/doc"
then xterm -e  \
   sh -c  'echo "Mounting the crypt...";\
           sudo ~toor/bin/mount-the-crypt ||  \
              read -p "problems!:"; \
           read -p ":";terminator&'
fi

{
# https://wiki.parabola.nu/GNOME_Keyring#Use_Without_Gnome 2021-05-20
# Start a dbus-session if not already running

source /etc/X11/xinit/xinitrc.d/80-dbus

pulseaudio  -D
#gentoo-pipewire-launcher &

# Start or connect Gnome-Keyring
eval "$(/usr/bin/gnome-keyring-daemon --start --daemonize --components=gpg,pkcs11,secrets,ssh)"

# You probably need to do this too:
export SSH_AUTH_SOCK
export GPG_AGENT_INFO
export GNOME_KEYRING_CONTROL
export GNOME_KEYRING_PID
}

/usr/libexec/mate-settings-daemon&
#xdaliclock -24 -cycle -bw 4 -transparent &
# { terminator --profile pretty || konsole || mate-terminal;} &
xscreensaver -no-splash&
pidof redshift || redshift &  # Adjust screen colortemp for night/day

## Browser
## Disabled to prevent writing the cache & profile
##directories before I can set ramdisks for them
## Chrome writes too often for disk's health
launch_chrome(){
 local c1 c2
  cntcro(){ ps -C chrome|wc -l; }

  if mountpoint "${HOME}/.config/google-chrome"
  then #xdg-open "http://localhost:8008" &
        xmonadctl -a goto 2
        xdg-open "http://localhost:8008" &
  fi

  for ((c1=0, c2=99; c1 != c2; ))
  do sleep 1
     c1=$(cntcro); c2=$(cntcro)
  done
}

xmonadctl -a goto 10
drracket &
# This hasn't workd yet but just in case it works someday
# The one lamp has been reassined from caps lock to be a
# numlok indicator
{ sleep 2; xkbcomp -I$HOME/.xkb $HOME/.xkb/symbols/numlock-led $DISPLAY ;} &

## Gettign a weird background. Trying to overide
fbsetbg -t "/export/images/public/Wallpaper/Hot Chix/tatoo-lady.jpg" &
{ sleep 16; fbsetbg -t "/export/images/public/Wallpaper/Hot Chix/tatoo-lady.jpg"; } &
{ sleep 1m; fbsetbg -t "/export/images/public/Wallpaper/Hot Chix/tatoo-lady.jpg"; } &
exit 0

